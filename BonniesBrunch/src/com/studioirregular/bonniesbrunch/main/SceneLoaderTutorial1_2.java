...
public class SceneLoaderTutorial1_2 extends SceneLoader {

	public SceneLoaderTutorial1_2(SceneManager manager, GameEntityRoot root) {
		super(manager, root);
	}
	...
	public void load() {
		...
		manager.add(loadBonnieTalks(mainGame, "tutorial_1_1_text_002", true));
		manager.add(loadCustomerAppears(mainGame, Customer.Type.WORKING_MAN));
		...
		manager.add(loadHintTapHoney(mainGame));
		manager.add(loadHoldAMoment(mainGame, 300, true));
		manager.add(loadHintDragBrunchToCustomer(mainGame, true));
		manager.add(loadWaitCustomerLeave(mainGame));
		manager.add(loadBonnieTalksGeneric(mainGame, "tutorial_1_1_text_004", true));
		
		manager.add(loadBonnieQuitTip(mainGame));
		manager.add(loadCustomerAppears(mainGame, Customer.Type.JOGGING_GIRL));
		manager.add(loadHoldAMoment(mainGame, 300, false));
		manager.add(loadHintTapHoney(mainGame));
		manager.add(loadHoldAMoment(mainGame, 300, true));
		manager.add(loadHintTapBagel(mainGame));
		...
		manager.add(loadHintDragBrunchToCustomer(mainGame, true));
		manager.add(loadWaitCustomerLeave(mainGame));
		manager.add(loadBonnieTalksGeneric(mainGame, "tutorial_1_1_text_005", true));
	}
	...
	private SceneNode loadBonnieTalksGeneric(RootMainGame mainGame, String talkTexture, boolean bonnieExcited) {
		SceneNode node = new SceneNode();
	...
	private SceneNode loadCustomerAppears(RootMainGame mainGame, Customer.Type type) {
		SceneNode node = new SceneNode();
		...
		orderSpec.addMustHave(Food.HONEY);
		CustomerSpec customerSpec = new CustomerSpec(type, orderSpec, CustomerSpec.Mode.Tutorial, 1);
	...
	private SceneNode loadHintTapHoney(RootMainGame mainGame) {
		SceneNode node = new SceneNode();
		
		RemoveEntity remove = new RemoveEntity(manager, "hold_a_moment");
		node.add(remove);
		
		Command command = new EnableTableItem(mainGame.table(), TableItem.HONEY, true);
		node.add(command);
		
		hintTap(node, false, 662, 296, 561, 221);
		
		final GameEvent trigger = GameEventSystem.getInstance().obtain(GameEvent.FOOD_GENERATED, Food.HONEY);
		final GameEvent nextNode = GameEventSystem.getInstance().obtain(GameEvent.SCENE_MANAGER_NEXT_NODE);
		node.addEventMap(trigger, nextNode);
	...
	private SceneNode loadHintDragBrunchToCustomer(RootMainGame mainGame, boolean removeHoldMoment) {
		SceneNode node = new SceneNode();
		...
		addAnim.addTranslateAnimation(375, 296, 375, 276, 300);
		addAnim.addTranslateAnimation(375, 276, 375, 296, 300);
		addAnim.addTranslateAnimation(375, 296, 270, 60, 1000);
		addAnim.addTranslateAnimation(270, 60, 270, 80, 300);
		addAnim.addTranslateAnimation(270, 80, 270, 60, 300);
		addAnim.addTranslateAnimation(270, 60, 270, 80, 300);
		addAnim.addTranslateAnimation(270, 80, 270, 60, 300);
		node.add(addAnim);
	...
	private SceneNode loadBonnieQuitTip(RootMainGame mainGame) {
		SceneNode node = new SceneNode();
		...
		Command command = new AddSimpleEntity(manager, "text", zOrderBase + 1,
				168, 241, 402, 124, "tutorial_1_2_text_001");
		node.add(command);
