...
public class SceneLoaderTutorial_2_1 extends SceneLoader {

	public SceneLoaderTutorial_2_1(SceneManager manager, GameEntityRoot root) {
		super(manager, root);
	}
	...
	public void load() {
		...
		manager.add(loadCustomerAppears(mainGame, Customer.Type.GIRL_WITH_DOG, 2, true));
		manager.add(loadHoldAMoment(mainGame, 500, false));
		
		manager.add(loadBonnieTalks(mainGame, "tutorial_2_1_text_002", false, true));
		manager.add(loadHintTapToastMachine(mainGame));
		manager.add(loadHoldAMoment(mainGame, 300, true));
		manager.add(loadHintCookToast(mainGame));
		manager.add(loadHoldAMoment(mainGame, 300, true));
		manager.add(loadWaitForFood(mainGame));
		manager.add(loadHoldAMoment(mainGame, 300, false));
		
		manager.add(loadHintTapToast(mainGame, true));
		manager.add(loadHoldAMoment(mainGame, 300, true));
		manager.add(loadBonnieTalks(mainGame, "tutorial_2_1_text_003", true, true));
		manager.add(loadHintTapLettuce(mainGame, true));
		manager.add(loadHoldAMoment(mainGame, 300, true));
		manager.add(loadBonnieTalks(mainGame, "tutorial_2_1_text_004", false, true));
		manager.add(loadHintTapToastTop(mainGame, true));
		manager.add(loadHoldAMoment(mainGame, 300, true));
	...
	private SceneNode loadCustomerAppears(RootMainGame mainGame, Customer.Type type, int seatIndex, boolean removeBonnieTalks) {
		...
		orderSpec.addMustHave(Food.TOAST_WHITE);
		orderSpec.addMustHave(Food.LETTUCE);
	...
	private SceneNode loadHintCookToast(RootMainGame mainGame) {
		SceneNode node = new SceneNode();
		
		RemoveEntity remove = new RemoveEntity(manager, "hold_a_moment");
		node.add(remove);
		
		Command command = new EnableTableItem(mainGame.table(), TableItem.TOAST_MACHINE, FoodMachine.ItemType.FoodButton, 0, true);
		node.add(command);
		
		hintTap(node, false, 278, 93, 227, 36);
		
		final GameEvent trigger = GameEventSystem.getInstance().obtain(GameEvent.FOOD_MACHINE_SLOT_FOOD_ADDED, Food.TOAST_WHITE);
		final GameEvent nextNode = GameEventSystem.getInstance().obtain(GameEvent.SCENE_MANAGER_NEXT_NODE);
		node.addEventMap(trigger, nextNode);
	...
	private SceneNode loadWaitForFood(RootMainGame mainGame) {
		SceneNode node = new SceneNode();
	...
	private SceneNode loadHintTapToast(RootMainGame mainGame, boolean removeHoldMoment) {
		SceneNode node = new SceneNode();
		...
		final GameEvent trigger = GameEventSystem.getInstance().obtain(GameEvent.FOOD_GENERATED, Food.TOAST_WHITE);
		final GameEvent nextNode = GameEventSystem.getInstance().obtain(GameEvent.SCENE_MANAGER_NEXT_NODE);
		node.addEventMap(trigger, nextNode);
	...
	private SceneNode loadHintTapLettuce(RootMainGame mainGame, boolean removeBonnieTalks) {
		SceneNode node = new SceneNode();
		...
		Command command = new EnableTableItem(mainGame.table(), TableItem.LETTUCE, true);
		node.add(command);
		
		hintTap(node, false, 417, 171, 366, 111);
		
		final GameEvent trigger = GameEventSystem.getInstance().obtain(GameEvent.FOOD_GENERATED, Food.LETTUCE);
		final GameEvent nextNode = GameEventSystem.getInstance().obtain(GameEvent.SCENE_MANAGER_NEXT_NODE);
		node.addEventMap(trigger, nextNode);
	...
	private SceneNode loadHintTapToastTop(RootMainGame mainGame, boolean removeBonnieTalks) {
		SceneNode node = new SceneNode();
